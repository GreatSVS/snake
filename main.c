...
/*
Задание выбора режима Бога, по умолчанию выключен
*/
uint8_t GM = 0;         //Количественное значение преимущества
uint32_t GT = 0;        //Тип игры


enum {LEFT=1, UP=2, RIGHT=3, DOWN=4, STOP_GAME='q'};
enum {MAX_TAIL_SIZE=1000, START_TAIL_SIZE=3, MAX_FOOD_SIZE=20, FOOD_EXPIRE_SECONDS=10, SPEED=20000, SEED_NUMBER=3};  //Add GM
...
void putFood(struct food f[], size_t number_seeds) {
    for(size_t i=0; i<number_seeds + GM; i++) {
        putFoodSeed(&f[i]);
    }
}
...
int main()
{
    ...
    printf("Choose type of game:\n");
    printf("1. Typical\n");
    printf("2.GOD mode\n");
    scanf("%" SCNu8, &GT);
    if (GT == 2) {
        GM = 7;             //Можно задавать кол-во преимущества
    } else {
        GM = 0;
    }
    timeout(100);
